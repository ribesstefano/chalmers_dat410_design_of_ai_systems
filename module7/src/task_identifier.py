...
task_dic = {nlp(' '.join([str(t) for t in weather_string if t.pos_ in ['NOUN', 'PROPN']])): WeatherTask,
            nlp(' '.join([str(t) for t in restaurant_string if t.pos_ in ['NOUN', 'PROPN']])): RestaurantTask,
...
class TaskIdentifier(object):
    ...
    def get_task_from_sentence(self, sentence):
        ...
        task_sentences = list(task_dic.keys())
        similarity_array = []
        ...
        print(task_sentences[index])

        return task_dic[task_sentences[index]]
